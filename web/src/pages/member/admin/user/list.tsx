...
const List = () => {
  ...
  const columns = React.useMemo<PrimaryTableCol[]>(
    () => [
      ...
      {
        colKey: 'nickname',
        title: translate('member.user.fields.nickname'),
        ellipsis: true,
        cell: ({ row }) => {
          return (
            <MediaText size='small'>
              <MediaText.Avatar image={row.avatar}></MediaText.Avatar>
              <MediaText.Title>{row.nickname}</MediaText.Title>
              <MediaText.Desc>{row.lebel_name}</MediaText.Desc>
            </MediaText>
          )
        },
      },
      {
        colKey: 'email',
        title: translate('member.user.fields.email'),
        ellipsis: true,
      },
      {
        colKey: 'tel',
        title: translate('member.user.fields.tel'),
        ellipsis: true,
      },
      {
        colKey: 'growth',
        title: translate('member.user.fields.growth'),
        ellipsis: true,
      },
      {
        colKey: 'status',
        title: translate('member.user.fields.status'),
        edit: {
          component: Switch,
          props: {},
          keepEditMode: true,
  ...
  return (
    <PageTable
      ...
      title={translate('member.user.name')}
      tabs={[
        {
          label: translate('member.user.tabs.all'),
          value: '0',
        },
        {
          label: translate('member.user.tabs.enabled'),
          value: '1',
        },
        {
          label: translate('member.user.tabs.disabled'),
          value: '2',
        },
      ]}
      actionRender={() => (
